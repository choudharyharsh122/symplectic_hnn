...
def objective(model, noisy_train_traj, noisy_val_traj, true_train_traj, true_val_traj, dt_gt, dt_solve, param_vals):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_data_loader:
            ...
            #y_pred_batch = solve_ivp_custom(model.forward, "forward", pq0_batch, (0, T), dt_solve, args=(model,), iters=5)
        ...
        with torch.no_grad():  # No gradient calculation during validation
            for batch in val_data_loader:
                ...
                #y_pred_batch_val = solve_ivp_custom(model.forward, "forward", pq0_batch, (0, T), dt_solve, args=(model,), iters=5)

                # Compute loss
                val_loss = criterion(y_pred_batch_val[:,1:,0], y_noisy_batch_val[:,1:,0]) + criterion(y_pred_batch_val[:,1:,1], y_noisy_batch_val[:,1:,1])
