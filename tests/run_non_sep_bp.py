...
def adjoint_ode(lam, args, kargs):
    ...
    #grad_h = torch.autograd.grad(outputs=h, inputs=y_tensor, grad_outputs=torch.ones_like(h), create_graph=True, retain_graph=True, allow_unused=True)[0]
    ...
    j_w = torch.stack((grad_m_0, grad_m_1), dim=2)
    ...
    lam_dot = - (torch.matmul(j_w, lam_tensor)).squeeze()
    #print("Done once")
...
# Calculate the integral involving the adjoint variable and gradients
def calculate_integral(model, y_values, T, lambda_values):
    ...
    for i, t in enumerate(t_eval):
        ...
        grad_w = []
        # Iterating over batch size and each element in the output gradient
        for k in range(w.shape[0]):  # 5
            grad_w_k = []
            for j in range(w.shape[1]):  # 2
                model.zero_grad()
                
                # Take the element of grad_model and set it to a scalar value
                grad_scalar = w[k, j]
                
                # Perform the backward pass
                y_tensor.requires_grad = False
                grad_scalar.backward(retain_graph=True)
                
                # Gather gradients
                param_grads = torch.cat([param.grad.flatten() for param in model.parameters() if param.grad is not None])
                
                # Append gradients for current (k, j) to grad_m_k
                grad_w_k.append(param_grads)
            
            # Stack gradients for the current i and append to grad_m
            grad_w.append(torch.stack(grad_w_k))

        # Stack to get the final tensor of shape (batch_size, 2, num_params)
        grad_w = torch.stack(grad_w)

        #print("Lambda Values shape: >>>>>>>>>>>", lambda_values.shape)
        #print("grad_w shape:>>>>>>>>>>>", grad_w.shape)

        integral_value = torch.matmul(torch.tensor(lambda_values[:, i, :], dtype=torch.float32).unsqueeze(-1).transpose(1,2), grad_w)
        
        integral_values.append(torch.mean(integral_value, dim=0))
        grad_vals.append(grad_w)

        # Clear the gradients for the next iteration
        model.zero_grad()

    integral_values = torch.stack(integral_values)

    #print("integral val i shape", integral_values[0,:].shape)


    integral_result = torch.trapz(integral_values, t_eval, axis=0).squeeze()
...
def load_data(datafolder, dt_solve, dt_gt):
    ...
    datafolder = "non_sep_10"

    noisy_train_path = "../data/"+str(datafolder)+"/noisy_non_sep_train.pt"
    noisy_val_path = "../data/"+str(datafolder)+"/noisy_non_sep_val.pt"
    noisy_test_path = "../data/"+str(datafolder)+"/noisy_non_sep_test.pt"

    train_path = "../data/"+str(datafolder)+"/non_sep_train.pt"
    val_path = "../data/"+str(datafolder)+"/non_sep_val.pt"
    test_path = "../data/"+str(datafolder)+"/non_sep_test.pt"
...
if __name__ == "__main__":
    ...
    input_size = 2
    output_size = 1
    noisy_train, noisy_val, true_train, true_val = load_data("non_sep", dt_gt, dt_solve)
    ...
    for i in range(len(params_list)):
        ...
        torch.save(model, f'../models/model_{i}_ns')
        df = pd.DataFrame({
        "train_loss": train_loss,
        "val_loss": val_loss
        })
        df.to_csv(f'output_{i}_ns.csv', index=False)
